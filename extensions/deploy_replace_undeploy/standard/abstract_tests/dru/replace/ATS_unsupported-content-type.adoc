[[ats_dru_replace_unsupported-media-type]]

[abstract_test]
====
[%metadata]
identifier:: /conf/dru/replace-unsupported-media-type
target:: /req/deploy-replace-undeploy/deploy-unsupported-media-type
...
3. Validate that the document contains the exception `type` “https://www.opengis.net/def/exceptions/ogcapi-processes-2/1.0/unsupported-media-type”.
--
====
