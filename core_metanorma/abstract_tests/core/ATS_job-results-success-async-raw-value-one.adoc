...
[requirement,type="abstracttest",label="/conf/core/job-results-async-raw-value-one"]
====
...
<<req_core_job-results-async-raw-value-one,/req/core/job-results-async-raw-value-one>>
