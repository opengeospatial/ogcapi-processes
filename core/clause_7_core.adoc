...
.link:https://raw.githubusercontent.com/opengeospatial/ogcapi-processes/master/core/openapi/schemas/qualifiedValue.yaml[The following is the schema for a qualified input value (quailifiedValue.yaml)]
[source,YAML]
----
...
[NOTE]
=====
In the following requirement the term `input value` is used to mean a single input value or a single value item in an array of input values (not to be confused with an array that is itself an input value).
=====

include::requirements/core/REQ_job-creation-input-inline-ref.adoc[]
...
.link:https://raw.githubusercontent.com/opengeospatial/ogcapi-processes/master/core/openapi/schemas/results.yaml[The following is the schema for the result of a job]
[source,YAML]
----
