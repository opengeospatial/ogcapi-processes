...
Input values can either be a single value or an array of values depending on
the multiplicty of the input specified using the `minOccurs` and `maxOccurs`
facets.  The following requirements apply:

include::requirements/core/REQ_job-creation-input-cardinality-eq1.adoc[]

include::requirements/core/REQ_job-creation-input-cardinality-gt1.adoc[]
