...
The OGC API - Processes standard is build upon the OGC API-Common standard. <<mapping-to-common>> Identifies the API-Common Requirements Classes which are applicable to each section of this standard.
...
.link:https://raw.githubusercontent.com/opengeospatial/ogcapi-processes/master/core/openapi/schemas/outputInfo.yaml[Schema for output info]
[source,YAML]
----
