...
The term process is one of the most used terms in both the information and geosciences domain. If not stated otherwise, this standard uses the term process as an umbrella term for any algorithm, calculation or model that either generates new data or transforms some input data into output data as defined in section 4.1 of the WPS 2.0 standard.
