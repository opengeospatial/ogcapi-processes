...
(in the context of OGC APIs) A mechanism defined by an OGC API Standard to access spatiotemporal data from a <<def-collection,collection>>/
...
A specific documented account of steps, roles, and rules involved in a computing process, often described using standard like the https://swagger.io/specification/[OpenAPI Standard].
...
[[def-remote-collections]]
==== remote collections

A <<def-collection,collection>> that is available from an external OGC API deployment different from the deployment whose process is being executed.
...
A server that implements one or more <<def-data-access-mechanism,data access mechanisms>> for accessing data from an <<def-collection,OGC collection>>.  The server acts as a source of values that are passed to a process being executed by an implementation of this Standard.
