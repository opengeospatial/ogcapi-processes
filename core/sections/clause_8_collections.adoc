...
The `passThroughParameters` parameter is a dictionary of query parameters that are passed directly to the value source server.  The keys used in this object should be identical to query parameters that the value source server recognizes.

In the case where the OAProc server supports local filtering, the `passThroughParameters` parameter provides a mechanism to force filtering to occur on the value source server.
...
The following requirements describe how an <<execute-request-body,execute request>> references a <<def-remote-collections,remote OGC API collection>> as a source of input values to a process.
...
See: <<sc-pass-though,Pass through parameters>>
