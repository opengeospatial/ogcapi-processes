...
The _Collection Input_ conformance class allows an <<def-collection,OGC API collection>>, available using one or more <<def-data-access-mechanism,OGC API data access mechanisms>>, to be specified as an input to a process.
...
execution of a process for a specific area, time and resolution of interest
...
pipeline. Parsing the <<execute-request-body,execute request>>, validating inputs, and handshakes with remote components 
only needs to be performed once. An implementation could also preempt future requests based on past requests,
offsetting the latency of a complex processing pipeline.
