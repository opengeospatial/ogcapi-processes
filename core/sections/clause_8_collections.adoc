...
Unless an implementation also conforms to the <<sc_local_filtering,_Local Filtering_>> conformance class, filtering of values retrieved from an <<def-collection,OGC API acessible collection>> is performed by the <<value-source-server,value source server>>.
