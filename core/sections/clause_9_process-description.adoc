...
If a server supports <<rc_collection-input,collection input>> and/or <<rc_collection-output,collection output>>, the `dataAccessAPIs` parameters in the process description allows the server to indicate, on a per-input/output basis, which <<def_data-access-mechanism,OGC API data access mechanism(s)>> are supported for providing input values to the process or retrieving output values from the process.
