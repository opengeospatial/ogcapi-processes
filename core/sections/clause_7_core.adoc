...
. Response headers: any additional https://datatracker.ietf.org/doc/html/rfc2616#section-14[HTTP headers] that the server should include in this context
...
. Req./Rec.: the relevant requirements/recommendations as per this specification
...
|See requirement: <<req_core_process-execute-sync-one-small,/req/core/process-execute-sync-one-small>>

|204 (_large_)
|_Link_ to output
|_n/a_
|_n/a_
|See requrement: <<req_core_process-execute-sync-one-big,/req/core/process-execute-sync-one-big>>
...
|See requirement: <<req_core_process-execute-async,/req/core/process-execute-async>>
|===
...
* The value *_standard_* in a means that the standard HTTP headers required in this context are generated for the response.  Any value other than *_standard_* indicates additional headers that should be included in the header.
* The value *_as negotiated_* means that the output content type is as negotiated between the client and the server based on the value of the HTTP Accept header.
====
...
[[schema_results]]
.Schema for a processing results presented as a JSON document
[source,yaml]
----
...
include::../requirements/core/REQ_process-execute-success-sync-one-small.adoc[]

include::../requirements/core/REQ_process-execute-success-sync-one-big.adoc[]
...
. *Response media type*: the value of the HTTP `Content-Type` header that the server should generate in this context
. *Response body*: a description of the content of the response body in this context
. *Req. or Rec.*: the requirement or recommendation as per this specification
