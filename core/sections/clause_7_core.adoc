...
Each process input is characterized by a schema, a cardinality and whether the input value can be passed by-value, by-reference or both.  The specific details of how these characteristics of a process input are specified in a process description is dependent on the particular process description language or vocabulary being used.

NOTE: In the case of the <<ogc_process_description,"OGC Process Description">> vocabulary, the `schema` parameter is used to define the schema of a process input, the cardinality of a process input is defined using the `minOccurs` and `maxOccurs` parameters and how a process input value can be passed in an execute request is specified using the `valuePassing` parameter.  See the <<ogc_process_description,"OGC Process Description">> clause for more details.
