...
* using the `profile` query parameter an `Accept` HTTP header if the media type includes provisions for such a parameter (e.g. `application/ld+json`)
* using the `Accept-Profile` header if the media type does not include provisions for such a parameter within the `Accept` header.
* using the `profile` query parameter with the https://www.rfc-editor.org/rfc/rfc7240[HTTP `Prefer` header]
...
* the `Preference-Applied` header with the `profile` query parameter should be returned if `Prefer` header was used to request it
