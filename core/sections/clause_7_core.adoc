...
NOTE: This is not always the case (e.g. `image/tiff; application=geotiff; profle=cloud-optimized`).  In such cases, the other the other headers (`Preference-Applied` and `Content-Type`) containing a `profile` parameter may be relevant.
