...
|Job results |`/processes/{processID}/jobs/{jobID}/results` |GET |<<sc_retrieve_job_results,7.14 Retrieve job results>>
!===
...
!===
|Description |Path |HTTP method | Parameter| Document reference
|Dismiss a job |`/processes/{processID}/jobs/{jobID}` |DELETE| Cancel job execution |<<sc_dismiss_job,7.13 Dismiss a job>>
!===
