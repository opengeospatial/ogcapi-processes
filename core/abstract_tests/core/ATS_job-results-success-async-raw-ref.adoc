...
. When the job status is `completed`, get the results as per test <<ats_core_job-results-op,/conf/core/job-results>> and verify that they conform to the test <<ats_core_job-results-async-raw-ref,/conf/core/job-results-async-ref>>.
|===
