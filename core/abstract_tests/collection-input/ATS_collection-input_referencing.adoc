...
test-purpose:: Verify that the server correctly processes a reference to a source collection from which input values are retrieved.
test-method::
+
--
...
4. Use the `collection` parameter in the <<execute-request-body,execute request>>, as defined in <<collection-value-schema,"collectionValue.yaml">>, to reference each source collection.
