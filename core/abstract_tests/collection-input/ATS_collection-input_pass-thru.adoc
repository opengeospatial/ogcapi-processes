...
test-purpose:: Verify that the server correctly processes the `passThruParameters` element in a collection input specification.
test-method::
+
--
...
4. In the specification of the collection input, specify the `passThruParameters` element, as defined in <<collection-value-schema,"collectionValue.yaml">>, and include query parameters specific to the selected <<def-data-access-mechanism,data access API>>.
