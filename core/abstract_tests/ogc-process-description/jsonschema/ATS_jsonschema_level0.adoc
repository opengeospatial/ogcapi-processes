[[ats_ogc-process-description_jsonschema_l0]]
[width="90%",cols="2,6a"]
|===
...
. For each input and output of each process, verify that the definition of the input or output conforms to the OpenAPI 3.0 schema https://raw.githubusercontent.com/opengeospatial/ogcapi-processes/master/core/openapi/schemas/schemaLevel0.yaml[schemaLevel0.yaml].
|===
