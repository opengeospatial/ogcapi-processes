...
test-purpose:: Validate that the response for multiple explicitly or implicitly requested outputs is a https://raw.githubusercontent.com/opengeospatial/ogcapi-processes/master/openapi/schemas/processes-core/results.yaml[results.yaml] document containing references to <<def-collection-description,collection description documents>>.
test-method::
+
--
...
6. Verify that the body of the response conforms to the https://raw.githubusercontent.com/opengeospatial/ogcapi-processes/master/openapi/schemas/processes-core/results.yaml[results.yaml] schema.
